...
class FontGlyphCpp:
    def __init__(self, glyph):
        ...
        if glyph.encoded_size_bytes > 0:
            ...
            self.array_declaration = f"static const uint8_t {self.array_name}[{glyph.encoded_size_bytes}] = {{{ data_body }}};"
        else:
        ...
        self.glyph_declaration = f"static const glyph::Glyph {self.glyph_name} = {{ {glyph.width}, {glyph.height}, {self.array_name}, {glyph.encoded_size_bytes}, glyph::{self.encoder_cpp_enum} }};"
...
static const glyph::Glyph* get_glyph(uint8_t c) {{
    ...
    while (low_i < high_i) {{
        ...
        const GlyphEntry& entry = glyph_entries[mid_i];
