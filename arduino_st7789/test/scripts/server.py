    ...
    async def launch_process(self, websocket):
        ...
        def parse_stdout():
            while True:
                ...
                header_format = "=IHHHHHHHHQBBI"
                header_size = struct.calcsize(header_format)
                header_data = proc.stdout.read(header_size)
                if len(header_data) != header_size:
                    break
                magic_number, x_start, x_end, y_start, y_end, x_cursor, y_cursor, image_width, image_height, time_nanos, brightness, hardware_reset, label_length = struct.unpack(header_format, header_data)
